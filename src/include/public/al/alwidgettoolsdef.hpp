...
#define SAFE_DELETE_LIST(p)     { if(p) {if(!((p)->isEmpty())) { for(int i = 0; i<(p)->size(); ++i){ (p)->removeAt(i);}(p)->clear();} SAFE_DELETE(p);}}
#endif
...
enum PositionPolicy {
	...
	Bottom      = 0x004,          // 0100
	Right       = 0x008,          // 1000
	TopRight    = Top | Right,    // 0001 | 1000 = 1001
	TopLeft     = Top | Left,     // 0001 | 0010 = 0011
	BottomRight = Bottom | Right, // 0100 | 1000 = 1100
	BottomLeft  = Bottom | Left   // 0100 | 0010 = 0110
};
...
enum ProgressType {
	...
	IndeterminateProgress = 0x002, // 进度条值不确定
	DiscontinuousLoading  = 0x003, // 间断加载动画(win11开机动画
	ContinuousLoading     = 0x004  // 连续加载动画
};
...
Q_ENUM_NS(ProgressTextPosition)
}
...
constexpr auto awesomeFontFamily = "CALAwesome";
...
inline QString getEnumTypeFontName(const std::type_info& typeInfo) {
	static const QMap<std::type_index, std::string> typeFontMap = {
		...
		{ typeid(FluentIcon), fluentFontFamily }
	};

	return QString::fromStdString(typeFontMap.value(typeInfo, errFontFamily));
}

inline QString getEnumTypeFontName(const ALIcon::IconType& iconType) {
	static const QMap<ALIcon::IconType, std::string> fontFamilyMap = {
		{ ALIcon::Awesome, awesomeFontFamily },
		{ ALIcon::Fluent, fluentFontFamily }
	};

	return QString::fromStdString(fontFamilyMap.value(iconType, errFontFamily));
}

template <typename T>
constexpr bool isAwesomeEnum() {
	return std::is_same_v<T, AweSomeIcon>;
}

template <typename T>
constexpr bool isFluentEnum() {
	return std::is_same_v<T, FluentIcon>;
}

template <typename EnumType>
QString getEnumTypeFontName(EnumType value) {
	return getEnumTypeFontName(typeid(EnumType));
}
}

/////////////////////////// alwidgettools Namespace end /////////////////////////////////////
