...
public:
	...
	void drawMessage(QPainter* painter, const QColor& backgroundColor, const QColor& iconColor, const QString& iconText, const QColor& textColor, int iconPixelSize,
	...
	static void showMessageBar(const UVMessageBarType::PositionPolicy& positionPolicy, const UVMessageBarType::MessageLevel& messageMode,
	                           const QString& title, const QString& message, int displayMsec, QWidget* parent = nullptr);

public slots:
	void slotCloseButtonClicked();
	void slotOtherMessageBarClosed(UVMessageBarType::PositionPolicy positionPolicy, int barIndex);

signals:
	void sigMessageBarClosed(UVMessageBarType::PositionPolicy positionPolicy, int messageBarIndex);
