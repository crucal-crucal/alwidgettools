 ...
 */
CALMenuStyle::CALMenuStyle(QStyle* style): QProxyStyle(style) {
	m_iconWidth = 22;
...
void CALMenuStyle::drawPrimitive(const PrimitiveElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const {
	switch (element) {
		case QStyle::PE_PanelMenu: {
			// 高性能阴影
			painter->save();
			painter->setRenderHint(QPainter::Antialiasing);
			ALTheme->drawEffectShadow(painter, option->rect, m_shadowBorderWidth, 6);
			// 背景绘制
...
void CALMenuStyle::drawControl(const ControlElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const {
	switch (element) {
		case QStyle::CE_MenuItem: {
			...
			if (const auto mopt = qstyleoption_cast<const QStyleOptionMenuItem*>(option)) {
				if (mopt->menuItemType == QStyleOptionMenuItem::Separator) {
					...
					painter->setBrush(ALThemeColor(m_themeMode, ALThemeType::BasicBaseLine));
					...
					if (mopt->state.testFlag(QStyle::State_Enabled) && (mopt->state.testFlag(QStyle::State_MouseOver) || mopt->state.testFlag(QStyle::State_Selected))) {
						...
						painter->setPen(Qt::NoPen);
						painter->setBrush(ALThemeColor(m_themeMode, ALThemeType::PopupHover));
...
int CALMenuStyle::pixelMetric(const PixelMetric metric, const QStyleOption* option, const QWidget* widget) const {
	switch (metric) {
...
QSize CALMenuStyle::sizeFromContents(const ContentsType type, const QStyleOption* option, const QSize& size, const QWidget* widget) const {
	switch (type) {
		case QStyle::CT_MenuItem: {
			if (const auto mopt = qstyleoption_cast<const QStyleOptionMenuItem*>(option)) {
				...
				const auto menu = dynamic_cast<const CALMenu*>(widget);
