 ...
 */
CALColorPicker::CALColorPicker(QWidget* parent): QWidget(parent) {
...
CALColorPicker::~CALColorPicker() = default;

void CALColorPicker::setThemeMode(const ALThemeType::ThemeMode& mode) {
	m_themeMode = mode;
}

ALThemeType::ThemeMode CALColorPicker::getThemeMode() const {
	return m_themeMode;
}

void CALColorPicker::setSelectedColor(const QColor& color) {
...
QColor CALColorPicker::getSelectedColor() const {
	return m_selectedColor;
}

void CALColorPicker::mousePressEvent(QMouseEvent* event) {
...
void CALColorPicker::mouseReleaseEvent(QMouseEvent* event) {
	m_selectedPoint = adjustPointLimit(event->pos());
	m_selectedColor = m_colorPickerImage.pixelColor(m_selectedPoint);
	Q_EMIT sigSelectedColorChanged(m_selectedColor);
...
void CALColorPicker::mouseMoveEvent(QMouseEvent* event) {
	m_selectedPoint = adjustPointLimit(event->pos());
	m_selectedColor = m_colorPickerImage.pixelColor(m_selectedPoint);
	Q_EMIT sigSelectedColorChanged(m_selectedColor);
...
void CALColorPicker::paintEvent(QPaintEvent* event) {
...
QPoint CALColorPicker::adjustPointLimit(QPoint point) const {
