 ...
 */
CALComboBoxStyle::CALComboBoxStyle(QStyle* style): QProxyStyle(style) {
...
void CALComboBoxStyle::drawPrimitive(const PrimitiveElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const {
	switch (element) {
...
void CALComboBoxStyle::drawControl(const ControlElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const {
	switch (element) {
		...
		case QStyle::CE_ShapedFrame: {
			// container区域
			if (widget->objectName() == "CALComboBoxContainer") {
				const QRect viewRect = option->rect;
				painter->save();
				painter->setRenderHints(QPainter::Antialiasing | QPainter::TextAntialiasing);
				ALTheme->drawEffectShadow(painter, viewRect, m_shadowBorderWidth, 6);
				...
				painter->setPen(ALThemeColor(m_themeMode, ALThemeType::PopupBorder));
				painter->setBrush(ALThemeColor(m_themeMode, ALThemeType::PopupBase));
			...
			if (const auto vopt = qstyleoption_cast<const QStyleOptionViewItem*>(option)) {
				...
				if (option->state.testFlag(QStyle::State_Selected)) {
					if (option->state.testFlag(QStyle::State_MouseOver)) {
						// 选中且鼠标悬停时
						painter->setBrush(ALThemeColor(m_themeMode, ALThemeType::BasicSelectedHoverAlpha));
						painter->drawPath(path);
					} else {
						// 选中但鼠标未悬停
						painter->setBrush(ALThemeColor(m_themeMode, ALThemeType::BasicSelectedAlpha));
						painter->drawPath(path);
					}
					painter->setPen(Qt::NoPen);
					painter->setBrush(ALThemeColor(m_themeMode, ALThemeType::PrimaryNormal));
					...
					if (option->state.testFlag(QStyle::State_MouseOver)) {
						// 未选中但鼠标悬停时
						painter->setBrush(ALThemeColor(m_themeMode, ALThemeType::BasicHoverAlpha));
						painter->drawPath(path);
					}
				}
#if 0
...
void CALComboBoxStyle::drawComplexControl(const ComplexControl control, const QStyleOptionComplex* option, QPainter* painter, const QWidget* widget) const {
	switch (control) {
		case QStyle::CC_ComboBox: {
			...
			if (const auto copt = qstyleoption_cast<const QStyleOptionComboBox*>(option)) {
				...
				painter->setBrush(ALThemeColor(m_themeMode, isEnabled ? copt->state.testFlag(QStyle::State_MouseOver) ? ALThemeType::BasicHover : ALThemeType::BasicBase : ALThemeType::BasicDisable));
...
QRect CALComboBoxStyle::subControlRect(const ComplexControl cc, const QStyleOptionComplex* opt, const SubControl sc, const QWidget* widget) const {
	switch (cc) {
		case QStyle::CC_ComboBox: {
			switch (sc) {
...
QSize CALComboBoxStyle::sizeFromContents(const ContentsType type, const QStyleOption* option, const QSize& size, const QWidget* widget) const {
	switch (type) {
...
void CALComboBoxStyle::setExpandIconRotate(const qreal ExpandIconRotate) {
	m_expandIconRotate = ExpandIconRotate;
	Q_EMIT sigExpandIconRotateChanged();
}

qreal CALComboBoxStyle::getExpandIconRotate() const {
	return m_expandIconRotate;
}

void CALComboBoxStyle::setExpandMarkWidth(const qreal ExpandMarkWidth) {
