...
void CALScrollBarStyle::drawComplexControl(const ComplexControl control, const QStyleOptionComplex* option, QPainter* painter, const QWidget* widget) const {
	switch (control) {
		case QStyle::CC_ScrollBar: {
			if (const auto sopt = qstyleoption_cast<const QStyleOptionSlider*>(option)) {
				...
				if (m_isExpand) {
					...
					painter->setPen(Qt::NoPen);
					painter->setBrush(ALThemeColor(m_themeMode, ALThemeType::BasicBase));
					painter->drawRoundedRect(scrollBarRect, 6, 6);
					// 指示器绘制
					constexpr int sideLength = 8;
					painter->setBrush(ALThemeColor(m_themeMode, ALThemeType::ScrollBarHandle));
...
int CALScrollBarStyle::pixelMetric(const PixelMetric metric, const QStyleOption* option, const QWidget* widget) const {
	switch (metric) {
...
int CALScrollBarStyle::styleHint(const StyleHint stylehint, const QStyleOption* opt, const QWidget* widget, QStyleHintReturn* returnData) const {
...
void CALScrollBarStyle::startExpandAnimation(const bool isExpand) {
	if (isExpand) {
		m_isExpand = true;
...
void CALScrollBarStyle::setOpacity(const qreal opacity) {
	m_opacity = opacity;
}

qreal CALScrollBarStyle::getOpacity() const {
	return m_opacity;
}

void CALScrollBarStyle::setSliderExtent(const qreal sliderExtent) {
	m_sliderExtent = sliderExtent;
}

qreal CALScrollBarStyle::getSliderExtent() const {
	return m_sliderExtent;
}

void CALScrollBarStyle::setIsExpand(const bool isExpand) {
	m_isExpand = isExpand;
}

bool CALScrollBarStyle::getIsExpand() const {
	return m_isExpand;
}

void CALScrollBarStyle::setScrollBar(CALScrollBar* scrollBar) {
