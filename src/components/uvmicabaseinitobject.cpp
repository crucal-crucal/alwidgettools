...
/**
 * @brief \class CUVMicaBaseInitObject
 * @param appPrivate private data
 * @param parent pointer to the parent class
 */
CUVMicaBaseInitObject::CUVMicaBaseInitObject(CUVApplicationPrivate* appPrivate, QObject* parent): QObject(parent), appPrivate(appPrivate) {
}

CUVMicaBaseInitObject::~CUVMicaBaseInitObject() = default;

void CUVMicaBaseInitObject::slotInitMicaBase(QImage image) {
	// QColorDialog
	// 统一处理为 1920 * 1080 以节省空间
	image = image.scaled(1920, 1080, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);

	QImage blurImage = CUVExponentialBlur::doExponentialBlur(image, 500).toImage();
