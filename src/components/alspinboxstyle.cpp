...
CALSpinBoxStyle::~CALSpinBoxStyle() = default;

void CALSpinBoxStyle::drawComplexControl(const ComplexControl cc, const QStyleOptionComplex* opt, QPainter* p, const QWidget* widget) const {
	switch (cc) {
		case CC_SpinBox: {
			if (const auto sopt = qstyleoption_cast<const QStyleOptionSpinBox*>(opt)) {
				...
				p->setBrush(ALThemeColor(m_themeMode, isEnable? ALThemeType::BasicBase : ALThemeType::BasicDisable));
				...
				if (isEnable) {
					if (sopt->activeSubControls == SC_ScrollBarAddLine) {
						if (sopt->state.testFlag(State_Sunken) && sopt->state.testFlag(State_MouseOver)) {
							...
							p->setBrush(ALThemeColor(m_themeMode, sopt->state.testFlag(QStyle::State_MouseOver) ? ALThemeType::BasicHoverAlpha : ALThemeType::BasicBaseDeep));
						}
					} else {
						p->setBrush(ALThemeColor(m_themeMode, ALThemeType::BasicBaseDeep));
					}
				} else {
					p->setBrush(ALThemeColor(m_themeMode, ALThemeType::BasicDisable));
				}
				...
				if (isEnable) {
					if (sopt->activeSubControls == SC_ScrollBarSubLine) {
						if (sopt->state.testFlag(State_Sunken) && sopt->state.testFlag(State_MouseOver)) {
							...
							p->setBrush(ALThemeColor(m_themeMode, sopt->state.testFlag(State_MouseOver) ? ALThemeType::BasicHoverAlpha : ALThemeType::BasicBaseDeep));
						}
					} else {
						p->setBrush(ALThemeColor(m_themeMode, ALThemeType::BasicBaseDeep));
					}
				...
				if (sopt->state.testFlag(State_HasFocus)) {
					p->setPen(QPen(ALThemeColor(m_themeMode, ALThemeType::PrimaryNormal), 2));
					p->drawLine(subLineRect.right() + 1, subLineRect.y() + subLineRect.height() - 2, addLineRect.left() - 1, subLineRect.y() + subLineRect.height() - 2);
				} else {
					p->setPen(ALThemeColor(m_themeMode, ALThemeType::BasicHemline));
...
QRect CALSpinBoxStyle::subControlRect(const ComplexControl cc, const QStyleOptionComplex* opt, const SubControl sc, const QWidget* widget) const {
