...
class CUVNavigationNode final : public QObject {
	Q_OBJECT
	Q_PROPERTY(QList<CUVNavigationNode*> childrenNodes READ getChildrenNodes WRITE setChildrenNodes NOTIFY sigChildrenNodesChanged)
...
public:
	explicit CUVNavigationNode(const QString& nodeTitle, CUVNavigationNode* parent = nullptr);
	~CUVNavigationNode() override;

	void setParentNode(CUVNavigationNode* parentNode);
	[[nodiscard]] CUVNavigationNode* getParentNode() const;
	...
	void addChildNode(CUVNavigationNode* childNode);

	[[nodiscard]] bool getIsChildHasKeyPoints() const;

	CUVNavigationNode* getOriginalNode();
	bool getIsChildNode(CUVNavigationNode* node);
