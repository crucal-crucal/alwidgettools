...
void CUVAwesomeToolButtonStyle::drawComplexControl(const ComplexControl cc, const QStyleOptionComplex* opt, QPainter* p, const QWidget* widget) const {
	switch (cc) {
...
QSize CUVAwesomeToolButtonStyle::sizeFromContents(const ContentsType ct, const QStyleOption* opt, const QSize& contentsSize, const QWidget* w) const {
...
void CUVAwesomeToolButtonStyle::drawIndicator(QPainter* painter, const QStyleOptionToolButton* bopt, const QWidget* widget) const {
...
void CUVAwesomeToolButtonStyle::drawIcon(QPainter* painter, QRect iconRect, const QStyleOptionToolButton* bopt, const QWidget* widget) const {
	if (bopt->toolButtonStyle != Qt::ToolButtonTextOnly) {
		const QSize iconSize = bopt->iconSize;
		if (widget->property("CUVIconType").toString().isEmpty()) {
			// 绘制QIcon
			...
			switch (bopt->toolButtonStyle) {
				case Qt::ToolButtonIconOnly: {
					iconFont.setPixelSize(0.75 * std::min(iconSize.width(), iconSize.height())); // NOLINT
					painter->setFont(iconFont);
					painter->drawText(iconRect, Qt::AlignCenter, widget->property("CUVIconType").toString());
					break;
				}
				case Qt::ToolButtonFollowStyle:
				case Qt::ToolButtonTextBesideIcon: {
					...
					iconFont.setPixelSize(0.75 * std::min(iconSize.width(), iconSize.height())); // NOLINT
					painter->setFont(iconFont);
					painter->drawText(adjustIconRect, Qt::AlignCenter, widget->property("CUVIconType").toString());
					break;
				}
				case Qt::ToolButtonTextUnderIcon: {
					if (bopt->features.testFlag(QStyleOptionToolButton::HasMenu) && !bopt->features.testFlag(QStyleOptionToolButton::MenuButtonPopup)) {
					...
					painter->setFont(iconFont);
					painter->drawText(adjustIconRect, Qt::AlignHCenter, widget->property("CUVIconType").toString());
					break;
				}
				default: {
					break;
...
qreal CUVAwesomeToolButtonStyle::calculateExpandIndicatorWidth(const QStyleOptionToolButton* bopt, QPainter* painter) {
	// 展开指示器
	const QSize iconSize = bopt->iconSize;
	painter->save();
	auto iconFont = QFont("CUVAwesome");
