 ...
 * Internal class for CUVColorDialog
 * @param q pointer to the public class
 * @param parent pointer to the parent class
 */
CUVColorDialogPrivate::CUVColorDialogPrivate(CUVColorDialog* q, QObject* parent): QObject(parent), q_ptr(q) {
}

CUVColorDialogPrivate::~CUVColorDialogPrivate() = default;

void CUVColorDialogPrivate::slotColorPickerColorChanged(const QColor& selectedColor) {
	Q_Q(CUVColorDialog);
...
void CUVColorDialogPrivate::slotColorValueSliderChanged(const int value) {
	Q_Q(CUVColorDialog);
...
void CUVColorDialogPrivate::slotColorModeChanged(const int index) {
	if (index == 0) {
		firstText->setText(tr("red"));
		secondText->setText(tr("green"));
		thirdText->setText(tr("blue"));
		firstEdit->setValidator(new CUVIntValidator(0, 255, this));
	} else if (index == 1) {
		firstText->setText(tr("hue"));
		secondText->setText(tr("saturation"));
		thirdText->setText(tr("value"));
		firstEdit->setValidator(new CUVIntValidator(0, 359, this));
	}
	updateEditValue();
}

void CUVColorDialogPrivate::slotHtmlEditFocusOut(const QString& text) const {
	// 自动补全
	if (text == "#") {
...
void CUVColorDialogPrivate::slotHtmlEditChanged(const QString& text) {
	if (text.length() > 1) {
		q_func()->setCurrentColor(QColor(completeColorText(text)));
	}
}

void CUVColorDialogPrivate::slotColorEditChanged(const QString& text) {
	q_func()->setCurrentColor(getColorFromEdit());
}

void CUVColorDialogPrivate::slotBasicColorViewClicked(const QModelIndex& index) {
	q_func()->setCurrentColor(index.data(Qt::UserRole).value<QColor>());
}

void CUVColorDialogPrivate::slotCustomColorViewClicked(const QModelIndex& index) {
	if (const auto color = index.data(Qt::UserRole).value<QColor>(); color.isValid()) {
		q_func()->setCurrentColor(color);
	}
}

void CUVColorDialogPrivate::slotAddCustomColorButtonClicked() const {
...
void CUVColorDialogPrivate::slotRemoveCustomColorButtonClicked() const {
	if (const QModelIndexList selectedIndexs = customColorView->selectionModel()->selectedIndexes(); !selectedIndexs.isEmpty()) {
		customColorModel->replaceDisplayColor({}, selectedIndexs[0].row());
	}
}

void CUVColorDialogPrivate::initBasicColor() const {
...
void CUVColorDialogPrivate::initCustomColor() const {
...
void CUVColorDialogPrivate::updateHtmlEditValue() const {
...
void CUVColorDialogPrivate::updateEditValue() const {
...
void CUVColorDialogPrivate::updateColorPreview() const {
	colorPreview->setBaseColor(currentColor);
	colorPreview->update();
}

void CUVColorDialogPrivate::updateColorValueSlider() const {
...
QString CUVColorDialogPrivate::completeColorText(QString text) {
	text.remove("#");
	return "#" + text.rightJustified(6, '0');
}

QString CUVColorDialogPrivate::getHexRgbValue() const {
...
QColor CUVColorDialogPrivate::getColorFromEdit() const {
	QColor editColor{};
...
Q_TAKEOVER_NATIVEEVENT_CPP(CUVColorDialog, d_func()->appBar)
/**
 * @brief \class CUVColorDialog
 * @param parent pointer to the parent class
 */
CUVColorDialog::CUVColorDialog(QWidget* parent): QDialog(parent), d_ptr(new CUVColorDialogPrivate(this, this)) {
	Q_D(CUVColorDialog);

	setFixedSize(620, 630);
	setObjectName("CUVColorDialog");
	setWindowTitle("CUVColorDialog");
	...
	d->colorPicker = new CUVColorPicker(this);
	connect(d->colorPicker, &CUVColorPicker::sigSelectedColorChanged, d, &CUVColorDialogPrivate::slotColorPickerColorChanged);
	...
	/// color preview
	d->colorPreview = new CUVColorPreview(this);
	...
	d->colorValueSliderStyle = new CUVColorValueSliderStyle;
	d->colorValueSlider->setStyle(d->colorValueSliderStyle);
	connect(d->colorValueSlider, &QSlider::valueChanged, d, &CUVColorDialogPrivate::slotColorValueSliderChanged);
	...
	/// color controller
	d->htmlEdit = new CUVLineEdit(this);
	d->htmlEdit->setText("#000000");
	const auto htmlValidator = new CUVIntValidator(0x000000, 0xFFFFFF, this);
	htmlValidator->setIsHexMode(true);
	d->htmlEdit->setValidator(htmlValidator);
	d->htmlEdit->setFixedSize(120, 33);
	connect(d->htmlEdit, &CUVLineEdit::sigFocusOut, d, &CUVColorDialogPrivate::slotHtmlEditFocusOut);
	connect(d->htmlEdit, &CUVLineEdit::textChanged, d, &CUVColorDialogPrivate::slotHtmlEditChanged);
	/// value preview
	...
	d->modeComboBox = new CUVComboBox(this);
	d->modeComboBox->setFixedSize(132, 33);
	d->modeComboBox->addItem("RGB");
	d->modeComboBox->addItem("HSV");
	d->modeComboBox->setCurrentIndex(0);
	connect(d->modeComboBox, QOverload<int>::of(&CUVComboBox::currentIndexChanged), d, &CUVColorDialogPrivate::slotColorModeChanged);
	/// R or H
	d->firstEdit = new CUVLineEdit(this);
	d->firstEdit->setText("0");
	d->firstEdit->setFixedSize(120, 33);
	d->firstEdit->setValidator(new CUVIntValidator(0, 255, this));
	connect(d->firstEdit, &CUVLineEdit::textEdited, d, &CUVColorDialogPrivate::slotColorEditChanged);
	d->firstText = new CUVText(tr("red"), this);
	...
	/// G or S
	d->secondEdit = new CUVLineEdit(this);
	d->secondEdit->setText("0");
	d->secondEdit->setFixedSize(120, 33);
	d->secondEdit->setValidator(new CUVIntValidator(0, 255, this));
	connect(d->secondEdit, &CUVLineEdit::textEdited, d, &CUVColorDialogPrivate::slotColorEditChanged);
	d->secondText = new CUVText(tr("green"), this);
	...
	/// B or V
	d->thirdEdit = new CUVLineEdit(this);
	d->thirdEdit->setText("0");
	d->thirdEdit->setFixedSize(120, 33);
	d->thirdEdit->setValidator(new CUVIntValidator(0, 255, this));
	connect(d->thirdEdit, &CUVLineEdit::textEdited, d, &CUVColorDialogPrivate::slotColorEditChanged);
	d->thirdText = new CUVText(tr("blue"), this);
	...
	/// base color
	d->basicColorView = new CUVBaseListView(this);
	...
	d->basicColorModel = new CUVColorDisplayModel(this);
	d->basicColorView->setModel(d->basicColorModel);
	d->basicColorDelegate = new CUVColorDisplayDelegate(this);
	d->basicColorView->setItemDelegate(d->basicColorDelegate);
	d->initBasicColor();
	connect(d->basicColorView, &CUVBaseListView::clicked, d, &CUVColorDialogPrivate::slotBasicColorViewClicked);
	const auto basicColorViewText = new CUVText(tr("basic color"), this);
	...
	/// custom color
	d->customColorView = new CUVBaseListView(this);
	...
	d->customColorModel = new CUVColorDisplayModel(this);
	d->customColorView->setModel(d->customColorModel);
	d->customColorDelegate = new CUVColorDisplayDelegate(this);
	d->customColorView->setItemDelegate(d->customColorDelegate);
	d->initCustomColor();
	connect(d->customColorView, &CUVBaseListView::clicked, d, &CUVColorDialogPrivate::slotCustomColorViewClicked);
	/// add & remove button
...
CUVColorDialog::~CUVColorDialog() = default;

void CUVColorDialog::setCurrentColor(const QColor& color) {
	Q_D(CUVColorDialog);
...
QColor CUVColorDialog::getCurrentColor() const {
	return d_func()->currentColor;
}

QList<QColor> CUVColorDialog::getCustomColorList() const {
	return d_func()->customColorModel->getDisplayColorList();
}

QColor CUVColorDialog::getCustomColor(const int index) const {
	return d_func()->customColorModel->getDisplayColor(index);
}

QString CUVColorDialog::getCurrentColorRgb() const {
	return d_func()->getHexRgbValue();
}

void CUVColorDialog::paintEvent(QPaintEvent* event) {
	Q_D(CUVColorDialog);
